...
namespace IntelliStretch.UI
{
    public partial class UIStretching : UserControl
    {
        ...
        private void Get_CurrentSettings(Protocols.StretchingProtocol protocol)
        {
            ...
            {
                protocol.FlexionVelocity = (int)sliderStretchExtV.Value;
                protocol.ExtensionVelocity = (int)sliderStretchFlexV.Value;
            ...
            {
                // UI components are flipped for JointSide == Left, so protocol logic is flipped for JointSide == Left
                protocol.FlexionVelocity = (int)sliderStretchExtV.Value;
                protocol.ExtensionVelocity = (int)sliderStretchFlexV.Value;
        ...
        private void Apply_StretchSettings(Protocols.StretchingProtocol protocol)
        {
            ...
            if (sp.IsConnected)
            {
                ...
                sp.WriteCmd($"DV{protocol.FlexionVelocity}"); // Stretching dorsi/flexion velocity limit
                sp.WriteCmd($"PV{protocol.ExtensionVelocity}");  // Stretching plantar/extension velocity limit
                sp.WriteCmd($"PT{protocol.ExtensionTorque}"); // Stretching dorsi/flexion torque limit
                sp.WriteCmd($"DT{protocol.FlexionTorque}"); // Stretching plantar/extension torque limit
            }
        }
