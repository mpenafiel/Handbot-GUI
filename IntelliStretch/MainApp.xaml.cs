    ...
    public partial class MainApp : Window
    {
        ...
        private void Initialize_Settings()
        {
            ...
            if (!File.Exists(defaultFile)) // If default file does not exsit
            {
                ...
                defaultProtocol.Wrist.Joint = Protocols.Joint.Wrist;
                defaultProtocol.Wrist.FlexionMax = 60;
                defaultProtocol.Wrist.ExtensionMax = -20;
                defaultProtocol.Wrist.ActiveFlexionMax = 60;
                defaultProtocol.Wrist.ActiveExtensionMax= -20;

                //MCP default settings
                defaultProtocol.MCP.Joint = Protocols.Joint.MCP;
                defaultProtocol.MCP.FlexionMax = 20;
                defaultProtocol.MCP.ExtensionMax = -20;
                defaultProtocol.MCP.ActiveFlexionMax = 20;
                defaultProtocol.MCP.ActiveExtensionMax = 20;
                ...
                // Vertical - All=0, Ankle=1;  Horizontal - Elbow=2, MCP=3, Wrist=4, Hand=5, Arm=6
        ...
        private void Restore_TorqueLimits()
        {
            if (sp.IsConnected)
            {
                sp.WriteCmd("DT" + intelliProtocol.Stretching.FlexionTorque.ToString()); // Stretching dorsi/flexion torque limit
                sp.WriteCmd("PT" + intelliProtocol.Stretching.ExtensionTorque.ToString()); // Stretching plantar/extension torque limit
            }
        }
